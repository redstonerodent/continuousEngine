...
class Go(Game):
    make_initial_state = lambda self: ('black', {t:0 for t in teams}, {t:set() for t in teams})

    def __init__(self, **kwargs):
        super().__init__(backgroundColor=Colors.background, name='continuous go', scale=50, **kwargs)

        self.save_state = lambda: (self.turn, self.capturedCount.copy(), {team:[p.loc.coords for p in self.layers[Layers.PIECES[team]]] for team in teams})
        self.load_state = lambda x: (lambda turn, capCount, pieces: (
            ...
            [self.clearLayer(Layers.PIECES[team]) for team in teams+['GHOST']],
            [self.makePiece(team, Point(*p)) for team in teams for p in pieces[team]],
        ...
        # debug
        self.edges = {t:set() for t in teams}
        self.components = {t:[] for t in teams}
        self.captures = set()
        ...
        self.removePieces = lambda ps: (lambda pcs: [(
            ...
            ])([pc for t in teams for pc in self.layers[Layers.PIECES[t]] if pc.loc in ps])
        ...
        self.piece_at = lambda pos: (lambda ps: ps[0] if ps else self.nextPiece)([p for t in teams for p in self.layers[Layers.PIECES[t]] if pos>>p.loc < piece_rad**2])
        ...
        self.keyPress[self.keys.clearGuides] = lambda _: ([setattr(p.guide, 'visible', False) for t in teams for p in self.layers[Layers.PIECES[t]]],
                                                            setattr(self.nextPiece.guide, 'visible', False),
    ...
    def attemptMove(self, move):
        ...
        [setattr(p.guide, 'visible', False) for t in teams for p in self.layers[Layers.PIECES[t]]]
    ...
    def updateMove(self, pos=None):
        ...
        if pos and on_board(pos):
            self.blockers = {p for p in sum((self.layers[Layers.PIECES[team]] for team in teams+['GHOST']),[]) if overlap(pos, p.loc)}

            opp = inc_turn[self.turn]
            pieces = {t:{p.loc for p in self.layers[Layers.PIECES[t]]} for t in teams}
    ...
    updateLiberties = lambda self: setattr(self, 'liberties', {pc.loc: (lambda close_pcs, close_opps: {tan for pc2 in close_pcs for tan in double_tangents(pc.loc, pc2) if on_board(tan) and not any(overlap(tan, p) for p in close_pcs) and connected(pc.loc, tan, close_opps)})([pc2.loc for t2 in teams for pc2 in self.layers[Layers.PIECES[t2]] if pc2 != pc and sorta_nearby(pc2.loc,pc.loc)], [pc2.loc for pc2 in self.layers[Layers.PIECES[inc_turn[t]]] if sorta_nearby(pc2.loc, pc.loc)]) for t in teams for pc in self.layers[Layers.PIECES[t]]})

    def updateGraph(self):
        pieces = {t:[p.loc for p in self.layers[Layers.PIECES[t]]] for t in teams}
        w_es = {t:weak_edges(pieces[t]) for t in teams}
        self.edges = {t:filter_edges(w_es[t], w_es[inc_turn[t]] | boundary_cuts(pieces[inc_turn[t]])) for t in teams}
        self.potentialEdges = {t:w_es[t]-self.edges[t] for t in teams}
        self.components = {t:components(pieces[t], list(self.edges[t])) for t in teams}

    def updateTerritory(self):
        self.territory = {t:sum(intersect_polygon_circle_area(self.voronoi.diagram.voronoi_vertices[pc.loc], Point(0,0), board_rad) for pc in self.layers[Layers.PIECES[t]]) / pi * piece_rad**2 for t in teams}
