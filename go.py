...
class Layers:
    BOUNDARY    = 1
    PIECES      = {'WHITE':2, 'BLACK':2.1, 'NEW':5}
    GUIDES      = 3
    COUNT       = 10
    DEBUG       = 20
...
game.load_state = lambda x: (lambda turn, capCount, pieces: (
    [game.clearLayer(Layers.PIECES[team]) for team in teams],
...
Circle(game, Layers.BOUNDARY, Colors.boundary, 0, 0, board_rad, 3).GETcolor = lambda g: Colors.boundary if game.mousePos == None or on_board(game.mousePos) else Colors.blocker
...
game.removePieces = lambda ps: (lambda pcs:[(game.layers[Layers.PIECES[pc.team]].remove(pc), game.capturedCount.__setitem__(pc.team, game.capturedCount[pc.team]+1)) for pc in pcs])([pc for t in teams for pc in game.layers[Layers.PIECES[t]] if pc.loc in ps])

nextPiece = GoPiece(game, 'NEW', None, None)
...
nextPiece.GETborder_color = lambda g: Colors.blocker if g.blockers or not on_board(game.mousePos) else Colors.capture if game.mousePos in game.captures else Colors.legal
...
def updateMove(game, pos=None):
    ...
    if pos and on_board(pos):
        game.blockers = {p for p in sum((game.layers[Layers.PIECES[team]] for team in teams),[]) if overlap(pos, p.loc)}
...
# if there are more MOUSEMOTION events in the queue, go through them only running the right click (pan) handler, and skip updateMove
